...
class FunctionalUnit:
    ...
    def issue(self, instruction, register_status):
        ...
        if instruction.repr.split()[0][0] is 'B':
            ...
            #setting all the instruction Register dependencies to the FU's dependencies
    ...
    def runtime(self, mem, reg,cdb):
        #function to perform actual instruction
        command = command_list.get(self.repr[0])
        command(self, mem, reg, cdb)

"""All of the Arithmetic and Branch Comparing operations to update the
registers and detect taken branches. Follows similar procedure from decode
with variable functions are assigned to each opcode to execute. These are
executed in the background during the read stage. However the value is
stored and isn't written to memory or a registered until the WRITE stage is
called.
"""
